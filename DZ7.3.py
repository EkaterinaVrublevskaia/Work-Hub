...
class Cell:
    ...
    def __add__(self, other):
        return f"Размер клетки при объединении 2 клеток: {self.c + other.c}"

    def __floordiv__(self, other):
        return f"Размер клетки при делении клеток друг на друга: {self.c // other.c}"

    def __sub__(self, other):
        return f"Размер клетки при вычитании одной от другой клетки: {self.c - other.c}"

    def __mul__(self, other):
        return f"Размер клетки при умножении клеток: {self.c * other.c}"

    def __mod__(self, other):
        return f"Остаток от деления клеток: {self.c % other.c}"

    def __pow__(self, other):
        return f"Размер клетки при возведении в степень: {self.c ** other.c}"

    def make_order(self, raw):
        res = ''
        for el in range(int(self.c / raw)):
            res += "*" * raw + "\n"
        res += "*" * (self.c % raw) + "\n"
        return res
